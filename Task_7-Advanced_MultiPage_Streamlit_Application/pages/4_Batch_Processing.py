...
st.markdown("## 📁 Bulk Batch Ingestion & Cohort Report Generator")
st.markdown("Upload bulk patient CSV datasets to execute batch PyTorch forward-pass inferences and generate exportable clinical reports.")
    ...
    try:
        ...
        if st.button("⚡ Execute Batch Inferences"):
            with st.spinner(f"🧠 Running batch PyTorch forward passes on {len(df_in)} records..."):
                ...
                with c1:
                    st.markdown("#### 🥧 Cohort Risk Distribution Breakdown")
                ...
                with c2:
                    st.markdown("#### 📥 Download Clinical Prediction Report")
